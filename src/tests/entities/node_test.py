...
class TestNode(unittest.TestCase):
    def setUp(self):
        ...
        self.graph.set_start_node(self.node_start)
    ...
    def test_path_from_start_returns_empty_list_when_no_path_from_start(self):
        graph = StubGraph()
        node_a = graph.create_node((0, 0))
        node_b = graph.create_node((1, 0))
        node_c = graph.create_node((10, 10))
        node_b.previous = node_a
        graph.set_start_node(node_c)

        self.assertEqual(node_b.path_from_start(), [])
